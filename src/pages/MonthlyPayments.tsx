...
export default function MonthlyPayments() {
  ...
  const [payments, setPayments] = useState<Payment[]>(() =>
    isSouthdale
      ? DEFAULT_SOUTHDALE.map((p) => ({ ...p, id: crypto.randomUUID() }))
      : []
  );
  ...
  function togglePaid(id: string) {
    setPayments((prev) =>
      prev.map((p) =>
        p.id === id ? { ...p, status: p.status === 'paid' ? 'planned' : 'paid' } : p
      )
    );
  }
  ...
  const freqBadge = (f: Frequency) => {
    const map: Record<Frequency, string> = {
      ...
      'bi-annual': 'bg-purple-100 text-purple-700',
      'once-off': 'bg-blue-100 text-blue-700',
      unforeseen: 'bg-amber-100 text-amber-800',
    };
    return map[f];
  };
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl">
      ...
      <p className="text-sm text-slate-500 mb-6">
        {organisation?.name} • Pre-loaded list + unforeseen / bi-annual / once-off
      </p>
      ...
      <div className="flex flex-wrap items-center gap-4 mb-4">
        ...
        <div className="text-sm text-slate-600">
          Planned total: <strong>R {totalPlanned.toFixed(2)}</strong>
          {' · '}
          Paid: <strong>R {totalPaid.toFixed(2)}</strong>
        </div>
        <button
          onClick={() => setShowAdd(true)}
          className="ml-auto flex items-center gap-1 text-sm bg-slate-800 text-white px-3 py-1.5 rounded-lg"
        >
      ...
      {showAdd && (
        <div className="bg-white border rounded-xl p-4 mb-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={addPayment} className="text-sm bg-emerald-600 text-white px-3 py-1.5 rounded-lg">
              Save
            </button>
            <button onClick={() => setShowAdd(false)} className="text-sm border px-3 py-1.5 rounded-lg">
              Cancel
            </button>
          </div>
        </div>
      ...
      <section className="bg-white border rounded-xl overflow-hidden">
            ...
            {payments.map((p) => (
              <tr key={p.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <span className={`text-xs px-2 py-0.5 rounded ${freqBadge(p.frequency)}`}>
                    {p.frequency}
                  </span>
                </td>
        ...
        {payments.length === 0 && (
          <p className="p-4 text-sm text-slate-400">No payments yet. Click “Add payment” to start.</p>
        )}
      </section>

      <p className="text-xs text-slate-400 mt-4">
        Southdale list is pre-loaded from Karren’s prioritised email. Add unforeseen, bi-annual or once-off
        items as needed. All amounts feed into the Monthly Control Report.
      </p>
    </div>
  );
}
